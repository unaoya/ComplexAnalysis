...
コーシーの積分定理を用いて領域が適切な条件を満たすとき原始関数の存在を証明する。
...
なので、この場合にはコーシーの積分定理は成立せず、実際線積分が経路によるような関数\\(f\\)が存在する。

## 積分路と原始関数

被積分関数\\(f\\)が原始関数\\(F\\)を持つとき、線積分の値が積分路によらず始点と終点のみで決まる。
（これは領域の形にはよらない？）
