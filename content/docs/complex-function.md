...
{{< katex >}}f{{< /katex >}}が\\(U\subset \mathbb{C}\\)で定義された複素関数であることを\\(f:U\to \mathbb{C}\\)と表す。
...
指数法則と等角性を用いて拡張する。
まず、指数法則から\\(\exp(x+yi)=\exp(x)\exp(yi)\\)となり、実数に対する\\(\exp(x)=e^x\\)はすでに定義されているため、
\\(\exp(yi)\\)を定義すればよい。

ここで、\\(\exp(x)\\)は\\(x\\)を動かすと実数倍されるため、その軌跡は原点から放射状に伸びる半直線になる。
これらの半直線と常に直交する曲線は何か？
それは原点を中心とした円である。
